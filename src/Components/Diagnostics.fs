...
<!-- Use `Ctrl+P > "F#: Get FSAC logs"` commands to get file location -->
    ...
    let writeToFile (text : string) =
        promise {
            let path = node.path.join(workspace.rootPath, "Diagnostric info")
    ...
    let getIonideLogs () =
        let writeStream =
            node.path.join(node.os.tmpdir(), "ionide", "FSAC_logs")
            |> Environment.ensureDirectory
            |> fun dir -> node.path.join(dir, DateTime.Now.ToString("yyyyMMdd-HHmmss.log"))
